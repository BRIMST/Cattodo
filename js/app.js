...
let userLocation = localStorage.getItem('user_location') || 'bogota';

function getShippingBadgesHTML(price) {
  const now = new Date();
  const hours = now.getHours();
  const day = now.getDay();
  let html = '<div class="product-shipping-badges">';

  if (userLocation === 'bogota') {
    // Caso 1: Bogotá
    html += `
      <div class="badge-shipping free">
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M1 3h15v13H1zM16 8h4l3 3v5h-7M5 18a2 2 0 1 0 0-4 2 2 0 0 0 0 4zM18 18a2 2 0 1 0 0-4 2 2 0 0 0 0 4z"/></svg>
        Envío GRATIS
      </div>`;
    ...
    if (day !== 0 && hours < 12) {
      html += `
        ...
        <div class="urgency-time-limit">Compra antes de las 12 PM</div>`;
    } else {
      const nextDay = (day === 6 || day === 0) ? 'el lunes' : 'mañana';
      html += `
        <div class="badge-shipping info">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M21 8V21H3V8M1 3h22v5H1zM10 12h4"/></svg>
          Llega ${nextDay}
        </div>`;
    }
  } else {
...
function openLocationModal() {
  safeStyle('modal-location', 'display', 'flex');
}
...
function setUserLocation(loc) {
  ...
  renderProducts(); // Re-render badges
}
...
function setWAProductContext(name, price) {
  const waBtn = document.getElementById('global-wa-btn');
  if (!waBtn) return;
  const phone = settings.whatsapp || '';
  const text = encodeURIComponent(`Hola, quiero este producto 👀: ${name} - ${settings.currency} ${parseFloat(price).toLocaleString('es-CO')}`);
      ...
      <div class="product-bottom-section">
        ...
        <div class="qty-controls">
          ${cart[p.id] ? `
            ...
            <button class="btn-qty" onclick="updateCart('${p.id}', 1)">+</button>
          ` : `<button class="btn-add ${hasDiscount ? 'btn-add-sale' : ''}" onclick="updateCart('${p.id}', 1); setWAProductContext('${p.name.replace(/'/g,"\\'")}', '${p.price}')">LO QUIERO</button>`}
        </div>
      </div>
    </div>
  `;
    ...
    return `
      <div class="discount-card">
        <div class="discount-card-img">
          ${mainImg ? `<img src="${mainImg}" loading="lazy" />` : '<span style="font-size:1.5rem">📦</span>'}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Detección de ubicación simple
  function detectLocation() {
    // Si el usuario ya eligió manualmente, no hacemos nada
    ...
    try {
      fetch('https://ipapi.co/json/')
        ...
        .then(data => {
          if (data && data.region) {
            const isBgt = data.region.toLowerCase().includes('bogot') || data.city.toLowerCase().includes('bogot');
            if (isBgt) {
               setUserLocation('bogota');
            } else {
               setUserLocation('nacional');
            }
          }
        }).catch(() => {});
    } catch(e) {}
  ...
  // ====== START ======
  // updateLocationUI(); // Desactivado
  // detectLocation(); // Desactivado
  updateWAButtonPosition();
  init();
