...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function detectLocation() {
    ...
    // Fix: ipapi.co bloqueaba CORS — reemplazado por ipwho.is (sin restricciones)
    const tryFetch = (url, mapFn) =>
      fetch(url).then(r => r.json()).then(mapFn);

    const providers = [
      {
        url: 'https://ipwho.is/',
        map: d => ({ region: d.region || '', city: d.city || '' })
      },
      {
        url: 'https://freeipapi.com/api/json',
        map: d => ({ region: d.regionName || '', city: d.cityName || '' })
      }
    ];
      ...
      tryFetch(url, map)
        .then(data => {
          ...
          if (data && (data.region || data.city)) {
            ...
            if (newLoc !== userLocation) setUserLocation(newLoc);
          } else {
            attempt(index + 1);
          }
        })
