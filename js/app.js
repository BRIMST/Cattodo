...
// Utility to compress images (Crucial for Firebase Realtime Database performance)
    ...
    els.productsGrid.innerHTML = filtered.map(p => {
      ...
      <div class="product-card">
        <div class="product-image-container" ${mainImg ? `onclick="openImageModal('${p.id}')" style="cursor:pointer;"` : ''}>
          ${mainImg ? `<img src="${mainImg}" style="width:100%;height:100%;object-fit:cover;" loading="lazy" />` : '📦'}
...
function openAdmin() {
  ...
  // Load orders ONLY when admin panel is opened (performance optimization)
  ...
  list.innerHTML = products.map(p => {
    ...
    <div class="admin-product-row" onclick="openProductModal('${p.id}')">
      <div class="admin-product-img">
        ${mainImg ? `<img src="${mainImg}" style="width:100%;height:100%;object-fit:cover;border-radius:8px;" loading="lazy" />` : '📦'}
      </div>
...
document.getElementById('qr-file-input').addEventListener('change', function(e) {
  if (e.target.files && e.target.files[0]) {
    ...
    reader.onload = async (ev) => {
      ...
      document.getElementById('settings-qr-preview').src = compressed;
...
document.getElementById('logo-file-input').addEventListener('change', function(e) {
  if (e.target.files && e.target.files[0]) {
    ...
    reader.onload = async function(evt) {
      const compressed = await compressImage(evt.target.result, 400, 0.8);
      const img = document.getElementById('settings-logo-preview');
      img.src = compressed;
      img.style.display = 'block';
      document.getElementById('logo-upload-placeholder').style.display = 'none';
