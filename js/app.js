...
function initCriticalApp() {
  ...
  on('btn-view-order', 'onclick', () => {
    // GA: usuario entra al carrito/pedido
...
function initSecondaryApp() {
  // ====== LISTENERS SECUNDARIOS (Admin, Ticket, Modals) ======
  const on = (id, event, fn) => {
    const el = document.getElementById(id);
    if (el) el[event] = fn;
  };
  ...
  on('btn-download-ticket', 'onclick', async () => {
    ...
    // GA: descarga de ticket
    trackEvent('descargar_ticket', 'Descargar comprobante');
  ...
  on('btn-open-admin', 'onclick', () => {
    // GA: acceso al panel admin
    trackEvent('click_boton', 'Abrir panel admin');
  ...
  on('btn-generate-ticket', 'onclick', () => {
    ...
    // GA: usuario inició proceso de compra con datos completos
    trackEvent('generar_ticket', 'Formulario de pedido completado');
    const customer = {
      name: nameVal,
      phone: phoneVal,
    ...
    const items = Object.entries(cart).map(([cartId, q]) => {
      const parts = cartId.split(':');
      const productId = parts[0];
      // Fix: producto simple no tiene variante — parts[1] es undefined
      const variantColor = parts.length > 1 ? (parts[1] || null) : null;
      const p = products.find(x => x.id === productId);
      // Fix: limpiar undefined del objeto producto antes de enviarlo a Firebase
      const cleanProduct = Object.fromEntries(
        Object.entries(p || {}).filter(([, v]) => v !== undefined)
      );
      return { ...cleanProduct, qty: q, variantColor };
    });
    ...
    push(ref(db, 'orders'), orderData).then((newRef) => {
      ...
      if (waBtn && settings.whatsapp) {
        ...
        waBtn.onclick = async (e) => {
          e.preventDefault();
          // GA: conversión principal — clic en WhatsApp para comprar
          trackEvent('click_whatsapp', 'Abrir WhatsApp para comprar', {
      ...
      if (shareBtn && navigator.share) {
        ...
        shareBtn.onclick = async () => {
          // GA: compartir ticket
          trackEvent('compartir_ticket', 'Compartir comprobante');
          try {
  ...
  if (locationDeptEl && typeof COLOMBIA_LOCATIONS !== 'undefined') {
    locationDeptEl.innerHTML = '<option value="">Selecciona departamento...</option>';
    Object.keys(COLOMBIA_LOCATIONS).sort().forEach(dept => {
      const opt = document.createElement('option');
      opt.value = dept;
      opt.textContent = dept;
      locationDeptEl.appendChild(opt);
    });
    locationDeptEl.addEventListener('change', () => {
      const cities = COLOMBIA_LOCATIONS[locationDeptEl.value] || [];
      locationCityEl.innerHTML = '<option value="">Selecciona ciudad...</option>';
      cities.forEach(city => {
        const opt = document.createElement('option');
        opt.value = city;
        opt.textContent = city;
        locationCityEl.appendChild(opt);
      });
      locationCityEl.disabled = cities.length === 0;
      updateShippingDisplay(locationDeptEl.value);
    });
  ...
  function detectLocation() {
    ...
    // Usar API de geolocalización del navegador (GPS/red) — sin CORS, sin API key
    ...
    navigator.geolocation.getCurrentPosition(
      (pos) => {
        ...
        // Nominatim (OpenStreetMap) — gratuito, sin CORS, sin API key
      ...
      () => {
        // Usuario denegó permiso o error — no forzar, dejar como nacional
        if (!userLocation) setUserLocation('nacional');
      },
      { timeout: 5000, maximumAge: 3600000 } // caché de 1 hora
    );
  }

  // ====== START ======
  // Fix: geolocalización diferida — se pide solo en la primera interacción del usuario
  // Evita el warning de PageSpeed "solicita permiso al cargar la página"
  const requestLocationOnInteraction = () => {
    detectLocation();
    document.removeEventListener('click', requestLocationOnInteraction);
    document.removeEventListener('scroll', requestLocationOnInteraction);
    document.removeEventListener('touchstart', requestLocationOnInteraction);
  };
  document.addEventListener('click', requestLocationOnInteraction, { once: true, passive: true });
  document.addEventListener('scroll', requestLocationOnInteraction, { once: true, passive: true });
  document.addEventListener('touchstart', requestLocationOnInteraction, { once: true, passive: true });

  // updateWAButtonPosition e init ya son llamados en initCriticalApp()
  ...
  // applySettings se maneja mediante onValue en init()
}
