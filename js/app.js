...
function getProductHTML(p, badgeText = null) {
  ...
  return `
    <div class="product-image-container" ${mainImg ? `onclick="openImageModal('${p.id}')"` : ''} style="cursor:pointer;">
      ...
      ${mainImg ? `<img src="${mainImg}" alt="Comprar ${p.name} - Panda Venta" loading="lazy" />` : '<span style="font-size:2rem">📦</span>'}
    ...
    return `
      <div class="discount-card">
        <div class="discount-card-img">
          ${mainImg ? `<img src="${mainImg}" alt="Comprar ${p.name} - Panda Venta" loading="lazy" />` : '<span style="font-size:1.5rem">📦</span>'}
