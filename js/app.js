...
// Event Listeners
document.getElementById('btn-view-order').onclick = () => { switchView('order'); renderOrderList(); };
document.getElementById('btn-back-catalog').onclick = () => switchView('catalog');
document.getElementById('btn-open-admin').onclick = () => {
  if (settings.adminPassword) document.getElementById('modal-login').style.display = 'flex';
  else openAdmin();
};
document.getElementById('btn-login-submit').onclick = () => {
  if (document.getElementById('login-password').value === settings.adminPassword) {
...
document.getElementById('product-file-input').onchange = async (e) => {
...
document.getElementById('btn-save-product').onclick = async () => {
  ...
  if (window.currentEditId) update(ref(db, `products/${window.currentEditId}`), pData);
  else push(ref(db, 'products'), pData);
  document.getElementById('modal-product').style.display = 'none';
  showToast('Producto guardado');
};
document.getElementById('btn-delete-product').onclick = () => {
...
document.getElementById('btn-generate-ticket').onclick = () => {
  const customer = {
  ...
  if (!customer.name || !customer.phone) return showToast('Nombre y teléfono requeridos');
  ...
  push(ref(db, 'orders'), orderData).then(snap => {
    // Render ticket view
    document.getElementById('ticket-customer-info').innerHTML = `
      <div><b>Para:</b> ${customer.name}</div>
      <div><b>Tel:</b> ${customer.phone}</div>
      <div><b>Dir:</b> ${customer.address}, ${customer.city}</div>
    `;
    ...
    document.getElementById('ticket-payment-info').textContent = settings.paymentInfo;
    switchView('ticket');
    cart = {};
    updateCartUI();
  });
...
document.getElementById('btn-back-order').onclick = () => switchView('order');
document.getElementById('btn-new-order').onclick = () => switchView('catalog');

document.getElementById('product-price').oninput = formatInputCurrency;
document.getElementById('product-cost').oninput = formatInputCurrency;
let searchTimer;
