...
function setUserLocation(loc) {
  ...
  safeStyle('modal-location', 'display', 'none');
  updateLocationUI();
...
function updateLocationUI() {
  ...
  if (display) {
    display.textContent = userLocation === 'bogota' ? 'Bogotá' : 'Otras ciudades';
  }
}
      ...
      <div class="product-bottom-section">
        ...
        <div class="qty-controls">
          ${cart[p.id] ? `
            ...
            <button class="btn-qty" onclick="updateCart('${p.id}', 1)">+</button>
          ` : `<button class="btn-add ${hasDiscount ? 'btn-add-sale' : ''}" onclick="updateCart('${p.id}', 1)">🛒 Lo quiero</button>`}
        </div>
      </div>
    </div>
  `;
