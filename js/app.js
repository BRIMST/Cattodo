...
function setUserLocation(loc) {
  ...
  if (loc === 'bogota') {
    if (heroTitle) heroTitle.innerHTML = '¡Envío GRATIS hoy<br>en Bogotá! ⚡';
    if (heroSub) heroSub.textContent = 'Pide antes de las 12 PM y recibe tu pedido hoy mismo.';
    if (trustBar) {
      trustBar.innerHTML = `
    ...
    if (heroTitle) heroTitle.innerHTML = 'Lo que buscas,<br>al mejor precio';
    if (heroSub) heroSub.textContent = 'Compra fácil y rápido — envíos a toda Colombia';
    if (trustBar) {
      trustBar.innerHTML = `
...
function initSecondaryApp() {
  ...
  on('btn-generate-ticket', 'onclick', async () => {
    ...
    try {
      ...
      safeText('ticket-customer-address', `${customer.address}, ${customer.city} (${customer.dept})`);
      
      const listEl = document.getElementById('ticket-items-list');
      if (listEl) {
